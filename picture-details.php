...
function fetchData() {
    ...
    xhr.open('GET', `https://mehndipvc.com/mlm-app/api/fetch-image-pagination.php?cat_id=${catId}&page=${currentPage}&item=${itemsPerPage}`, true);
    xhr.onload = function() {
        if (this.status === 200) {
...
function displayData() {
    ...
    allData.forEach(item => {
        ...
        div.innerHTML = `
            <div class="data-item">
                <a data-fancybox="gallery" data-caption="${item.id}" href="https://mehndipvc.com/api/assets/${item.image}">
                    <img src="https://mehndipvc.com/api/assets/${item.image}" alt="${item.category}">
                </a>
                <div class="category-name">${item.category}</div>
                <div class="sl-name">${item.id}</div>
                <button class="share-button" onclick="shareImage('https://mehndipvc.com/api/assets/${item.image}')">
