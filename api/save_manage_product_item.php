...
function setData($input){
    ...
    $checkIfItemExist = mysqli_query($conn, "SELECT * FROM `sub_users_items` WHERE `user_id` = '$user_id' AND `parent_id` = '$parent_id' AND `item_id` = '$item_id'");
    if (mysqli_num_rows($checkIfItemExist) > 0) {
        $query="UPDATE `sub_users_items` SET `price`='$price', `status`='$status' WHERE `user_id` = '$user_id' AND `parent_id` = '$parent_id' AND `item_id` = '$item_id'";
    } else {
        $query="INSERT INTO `sub_users_items`(`user_id`, `parent_id`, `item_id`, `price`, `status`) VALUES ('$user_id','$parent_id','$item_id','$price','$status')";
    }
